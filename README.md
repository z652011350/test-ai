...
│       │   ├── SKILL.md                 # 技能定义（5 阶段）
│       │   ├── scripts/
│       │   │   ├── extract_kit_api.py   # 从 .d.ts 提取 API 声明
...
└── scripts/
    ├── kit-scan/                        # 流水线 A：单体模式
    │   ├── scan_kit.py                  # 入口脚本
    │   ├── batch_pipeline.py            # 数据分批、prompt 构建、结果合并
    │   ├── claude_runner.py             # Claude CLI 封装（无重试）
    │   ├── batch_scan_all.py            # 批量扫描所有 Kit
    │   └── kit_compont.csv              # Kit-部件映射
    └── kit-scan-test/                   # 流水线 B：Harness 模式
        ├── scan_kit.py                  # 入口脚本
        ├── data_prepare.py              # 数据合并、XLSX 转换
        ├── claude_runner.py             # Claude CLI 封装（3 次重试）
        ├── batch_scan_all.py            # 批量扫描所有 Kit
        └── kit_compont.csv              # Kit-部件映射
```
...
| Python 3.10+ | 运行流水线脚本 | `python3 --version` |
...
scan_kit.py
  ...
  │
  ├─ Step 2: batch_pipeline 按 batch_size=20 切分数据
  │
...
| `-out_path` | 是 | - | 输出根目录 |
| `-batch_size` | 否 | 20 | 每个 batch 的 API 数量 |
...
batch_scan_all.py                          # 从 kit_compont.csv 读取 Kit 列表
  ...
  └─ subprocess.run(scan_kit.py ...)       # 每个 Kit 调用一次
       │
       ├─ claude_runner.run_claude_command("/kit-api-extract ...")
       │    └─ subprocess.Popen(["claude", "-p", prompt, "--allowedTools", ...])
       │
       ├─ [流水线 A] batch_pipeline.prepare_batches()     # 切分 batch
       │    └─ claude_runner.run_batch_scan()              # 逐 batch 调用
       │         └─ run_claude_command("/api-level-scan ...") × N
...
两种流水线的 `claude_runner.py` 是唯一的 CLI 调用入口，只需修改此文件即可适配不同 Agent。

### 关键配置项

两个 `claude_runner.py`（`scripts/kit-scan/claude_runner.py` 和 `scripts/kit-scan-test/claude_runner.py`）中有 3 个核心常量：
...
# 仅 kit-scan-test/claude_runner.py 有（第 18 行）：
MAX_RETRIES: int = 3
```
...
### batch_scan_all.py 中的固定路径

`batch_scan_all.py` 中硬编码了以下路径，需根据实际环境修改（文件顶部第 23-26 行）：

| 常量 | 含义 |
|------|------|
| `JS_DECL_PATH` | interface_sdk-js 目录 |
| `REPO_BASE` | DataBases 目录 |
| `OUT_PATH` | 输出根目录 |
| `DOC_PATH` | API 文档目录 |
| `CSV_PATH` | kit_compont.csv（自动解析，一般无需修改） |
...
| 参数 | `kit_name`, `js_sdk_path`, `databases_dir`, `output_dir` |
| 输出 | `api.jsonl`（声明数据）+ `impl_api.jsonl`（含实现链路） |

执行 5 个阶段：
1. `extract_kit_api.py` 从 `.d.ts` 提取 API 声明
2. 按 module 分组，派发 Explore subagent（最多 3 并行）探索 NAPI→Framework→Implementation 链路
3. `merge_agent_results.py` 合并为 `impl_api.jsonl`
4. 重试：NAPI 覆盖率 < 90% 时再探索（最多 2 轮）
5. 生成 `api_extraction_report.md`
...
确认 Claude Code CLI 已安装并在 PATH 中：`which claude`。若安装在其他位置，修改 `claude_runner.py` 中的 `CLAUDE_CLI` 常量。
...
`claude_runner.py` 会自动重试 3 次，每次追加提示告知是重试。若仍失败，检查 Claude CLI 是否正常，或减少 `-max_parallel` 降低并发。
...
JSONL → XLSX 的转换逻辑在 `batch_pipeline.py`（流水线 A）或 `data_prepare.py`（流水线 B）的 `jsonl_to_xlsx()` 函数中，可自定义列名和格式。
